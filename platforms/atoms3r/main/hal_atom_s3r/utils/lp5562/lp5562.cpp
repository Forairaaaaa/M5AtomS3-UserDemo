...
bool LP5562_Class::begin()
{
    ...
    // Set chip_en to 1
    if (!_write_register(0x00, 0B01000000)) return false;
    _delay(1);
...
void LP5562_Class::reset()
{
    _write_register(LP5562_REG_RESET, LP5562_RESET);
}

void LP5562_Class::enable()
{
    _write_register(LP5562_REG_ENABLE, LP5562_ENABLE_DEFAULT);
}

void LP5562_Class::disable()
{
    _write_register(LP5562_REG_ENABLE, 0x00);
}

void LP5562_Class::runProgram()
{
    _write_register(LP5562_REG_ENABLE, LP5562_ENABLE_RUN_PROGRAM);
}

void LP5562_Class::stopProgram()
{
    _write_register(LP5562_REG_ENABLE, LP5562_ENABLE_DEFAULT);
}
...
/*                                    Port                                    */
/* -------------------------------------------------------------------------- */
bool LP5562_Class::_write_register(uint8_t reg, uint8_t value)
{
    return writeRegister8(reg, value);
}

uint8_t LP5562_Class::_read_register(uint8_t reg)
{
    return readRegister8(reg);
}

void LP5562_Class::_delay(uint32_t ms)
{
    m5gfx::delay(ms);
}
